...
class RainTPL{
	...
	// -------------------------
		...
		/**
		 * App assets path. RainTPL will add this URL to the relative paths of element selected in $path_replace_list.
		 *
		 * @var string
		 */
        ...
        static $left_reg_delimiter = "<\{";
        static $right_reg_delimiter = "\}>";
	...
	protected function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			$this->tpl['template_directory']    = self::$tpl_dir;								// template directory
			$this->tpl['tpl_filename']          = $this->tpl['template_directory'] . $tpl_name . '.' . self::$tpl_ext;	// template filename
			$temp_compiled_filename             = self::$cache_dir .dirname($tpl_name).'/'. $tpl_basename . "." . md5( $this->tpl['template_directory'] . serialize(self::$config_name_sum));
			...
			if(preg_match('/http/', $tpl_name)){
				$this->compileFile('', '', $tpl_name, self::$cache_dir.dirname($tpl_name), $this->tpl['compiled_filename'] );
				return true;
			}
			...
			elseif( !file_exists( $this->tpl['compiled_filename'] ) || ( self::$check_template_update && filemtime($this->tpl['compiled_filename']) < filemtime( $this->tpl['tpl_filename'] ) ) ){
				$this->compileFile( $tpl_basename, $tpl_basedir, $this->tpl['tpl_filename'], self::$cache_dir.dirname($tpl_name).'/', $this->tpl['compiled_filename'] );
				return true;
			}
	...
	*/
	...
	protected function compileTemplate( $template_code, $tpl_basedir ){
        self::$left_reg_delimiter = '\\'.join('\\',str_split(self::$left_delimiter));
        self::$right_reg_delimiter = '\\'.join('\\',str_split(self::$right_delimiter));
		//tag list
		$tag_regexp = array( 'loop'         => '('.self::$left_reg_delimiter.'loop(?: name){0,1}="\${0,1}[^"]*"'.self::$right_reg_delimiter.')',
                             'loop_close'   => '('.self::$left_reg_delimiter.'\/loop'.self::$right_reg_delimiter.')',
                             'if'           => '('.self::$left_reg_delimiter.'if(?: condition){0,1}="[^"]*"'.self::$right_reg_delimiter.')',
                             'elseif'       => '('.self::$left_reg_delimiter.'elseif(?: condition){0,1}="[^"]*"'.self::$right_reg_delimiter.')',
                             'else'         => '('.self::$left_reg_delimiter.'else'.self::$right_reg_delimiter.')',
                             'if_close'     => '('.self::$left_reg_delimiter.'\/if'.self::$right_reg_delimiter.')',
                             'function'     => '('.self::$left_reg_delimiter.'function="[^"]*"'.self::$right_reg_delimiter.')',
                             'noparse'      => '('.self::$left_reg_delimiter.'noparse'.self::$right_reg_delimiter.')',
                             'noparse_close'=> '('.self::$left_reg_delimiter.'\/noparse'.self::$right_reg_delimiter.')',
                             'ignore'       => '('.self::$left_reg_delimiter.'ignore'.self::$right_reg_delimiter.'|'.self::$left_reg_delimiter.'\*)',
                             'ignore_close'	=> '('.self::$left_reg_delimiter.'\/ignore'.self::$right_reg_delimiter.'|\*'.self::$right_reg_delimiter.')',
                             'include'      => '('.self::$left_reg_delimiter.'include=["\'][^"\']*["\'](?: cache=["\'][^"\']*["\'])?'.self::$right_reg_delimiter.')',
                             'template_info'=> '('.self::$left_reg_delimiter.'\$template_info'.self::$right_reg_delimiter.')',
                             'function'		=> '('.self::$left_reg_delimiter.'function="(\w*?)(?:.*?)"'.self::$right_reg_delimiter.')'
							);
	...
	protected function compileCode( $parsed_code ){
		...
		$compiled_code = $open_if = $comment_is_open = $ignore_is_open = null;
	 	...
	 	foreach( $parsed_code as $html ){
	 		...
	 		//close ignore tag
			if( !$comment_is_open && ( strpos( $html, self::$left_delimiter.'/ignore'.self::$right_delimiter ) !== FALSE || strpos( $html, '*'.self::$right_delimiter ) !== FALSE ) )
	 		...
	 		//close no parse tag
			elseif( strpos( $html, self::$left_delimiter.'/noparse'.self::$right_delimiter ) !== FALSE )
	 		...
	 		//ignore
			elseif( strpos( $html, self::$left_delimiter.'ignore'.self::$right_delimiter ) !== FALSE || strpos( $html, self::$left_delimiter.'*' ) !== FALSE )
	 			$ignore_is_open = true;
			...
			//include tag
			elseif( preg_match( '/'.self::$left_reg_delimiter.'include=["\']([^"\']*)["\'](?: cache=["\']([^"\']*)["\']){0,1}'.self::$right_reg_delimiter.'/', $html, $code ) ){
	 		...
	 		//loop
			elseif( preg_match( '/'.self::$left_reg_delimiter.'loop(?: name){0,1}="\${0,1}([^"]*)"'.self::$right_reg_delimiter.'/', $html, $code ) ){
			...
			//if
			elseif( preg_match( '/'.self::$left_reg_delimiter.'if(?: condition){0,1}="([^"]*)"'.self::$right_reg_delimiter.'/', $html, $code ) ){
			...
			//elseif
			elseif( preg_match( '/'.self::$left_reg_delimiter.'elseif(?: condition){0,1}="([^"]*)"'.self::$right_reg_delimiter.'/', $html, $code ) ){
			...
			//else
			elseif( strpos( $html, self::$left_delimiter.'else'.self::$right_delimiter ) !== FALSE ) {
			...
			//close if tag
			elseif( strpos( $html, self::$left_delimiter.'/if'.self::$right_delimiter ) !== FALSE ) {
			...
			//function
			elseif( preg_match( '/'.self::$left_reg_delimiter.'function="(\w*)(.*?)"'.self::$right_reg_delimiter.'/', $html, $code ) ){
			...
			// show all vars
			elseif ( strpos( $html, self::$left_delimiter.'$template_info'.self::$right_delimiter ) !== FALSE ) {

				//tag
				$tag  = self::$left_delimiter.'$template_info'.self::$right_delimiter;
			...
			//all html code
				...
				$html = $this->var_replace( $html, $left_delimiter = self::$left_reg_delimiter, $right_delimiter = self::$right_reg_delimiter, $php_left_delimiter = '<?php ', $php_right_delimiter = ';?>', $loop_level, $echo = true );
				//const substitution (es. {#CONST#})
				$html = $this->const_replace( $html, $left_delimiter = self::$left_reg_delimiter, $right_delimiter = self::$right_reg_delimiter, $php_left_delimiter = '<?php ', $php_right_delimiter = ';?>', $loop_level, $echo = true );
				//functions substitution (es. {"string"|functions})
				$compiled_code .= $this->func_replace( $html, $left_delimiter = self::$left_reg_delimiter, $right_delimiter = self::$right_reg_delimiter, $php_left_delimiter = '<?php ', $php_right_delimiter = ';?>', $loop_level, $echo = true );
			}
		}
	 ...
	 */
	protected function path_replace( $html){
		...
		if( self::$path_replace ){
			...
			if( in_array( "a", self::$path_replace_list ) ){
				$exp = array_merge( $exp , array( '/<a(.*?)href=(?:")(http\:\/\/|https\:\/\/|javascript:|mailto:)([^"]+?)(?:")/i','/<a(.*?)href="\/"/', '/<a(.*?)href="\/(.*?)"/', '/<a(.*?)href=(?:\@)([^"]+?)(?:\@)/i' ) );
				$sub = array_merge( $sub , array( '<a$1href=@$2$3@', '<a$1href="' . self::$base_url.'"', '<a$1href="' . self::$base_url . '/$2"', '<a$1href="$2"') );
			}

			if( in_array( "form", self::$path_replace_list ) ){
				$exp = array_merge( $exp , array( '/<form(.*?)action="\/"/', '/<form(.*?)action="\/(.*?)"/', '/<form(.*?)action=(?:\@)([^"]+?)(?:\@)/i' ) );
				$sub = array_merge( $sub , array( '<form$1action="' . self::$base_url.'"', '<form$1action="' . self::$base_url . '/$2"', '<form$1action="$2"') );
			}
	...
	function const_replace( $html, $tag_left_delimiter, $tag_right_delimiter, $php_left_delimiter = null, $php_right_delimiter = null, $loop_level = null, $echo = null ){
		// const
		return preg_replace( '/'.self::$left_reg_delimiter.'\#(\w+)\#{0,1}'.self::$right_reg_delimiter.'/', $php_left_delimiter . ( $echo ? " echo " : null ) . '\\1' . $php_right_delimiter, $html );
	}
	...
	function func_replace( $html, $tag_left_delimiter, $tag_right_delimiter, $php_left_delimiter = null, $php_right_delimiter = null, $loop_level = null, $echo = null ){
		...
		preg_match_all( '/'.self::$left_reg_delimiter.'\#{0,1}(\"{0,1}.*?\"{0,1})(\|\w.*?)\#{0,1}'.self::$right_reg_delimiter. '/', $html, $matches );
